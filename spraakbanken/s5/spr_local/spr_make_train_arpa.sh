...
for knd in $(seq ${order} -1 0); do
    ...
    cut -f2- -d" " $inf | $filter_cmd | ngram-count -memuse -read - -lm ${outdir}/arpa -write-vocab ${outdir}/vocab -order ${order} $MINCOUNT $INTERPOLATE $KNDISCOUNT $WBDISCOUNT || again=1
