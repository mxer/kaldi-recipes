...
if [ -d data-prep/corpus ]; then
    ok=0
    for f in "wav.ark" "wav.scp" "text.orig"  "utt2spk" "utt2comp"; do
...
extract-segments --max-overshoot=3.0 scp:${raw_files_dir}/sorted.scp ${raw_files_dir}/segments.sorted ark,scp:data-prep/corpus/wav.ark,data-prep/corpus/wav.scp
