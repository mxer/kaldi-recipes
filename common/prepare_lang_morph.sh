...
if $position_dependent_phones; then
  ...
  if "$silprob"; then
    ...
    perl -ane '@A=split(" ",$_); $w = shift @A; $p = shift @A; @A>0||die;
         ...
         for($n=1;$n<@A-1;$n++) { print "$A[$n]_I "; } print "$A[$n]_E\n"; } ' \
           < $srcdir/lexiconp.txt | sed "s/ DUMMY_[BESI]//g"  > $tmpdir/lexiconp.pre_reverse || exit 1;
    if $reverse; then
