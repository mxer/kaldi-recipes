...
srun -n1 steps/nnet3/decode.sh --iter 200 --beam 20.0 --lattice-beam 12.0 --skip-scoring $skip_scoring --nj 7 $decode_flags --scoring-opts "--min-lmwt 1" --online-ivector-dir ${am}/ivecs/ivectors_$(basename ${dataset})_hires ${am}/graph_${sname} ${dataset}_hires ${am}/decode200${dsextra}_${sname}
if [ $# -ge 3 ]; then

biglm=$3
bname=$(basename $biglm)
srun -n1 steps/lmrescore_const_arpa.sh --skip-scoring $skip_scoring $smalllm ${biglm} ${dataset}_hires ${am}/decode200${dsextra}_${sname} ${am}/decode200${dsextra}_${sname}_ca_${bname}
fi
