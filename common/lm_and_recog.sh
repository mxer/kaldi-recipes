...
if [ ! -f $lm_file ] || [ $(stat stat -c %s ${lm_file}) -le 32 ]; then
  job job vk_${small_contexts}M_${name} ${small_mem} 32 NONE -- \
...
if [ ! -f $fst_file ] || [ $(stat stat -c %s ${fst_file}) -le 32 ] || [ $fst_file -ot $lm_file ]; then
...
if [ ! -f $lm_file ] || [ $(stat stat -c %s ${lm_file}) -le 32 ]; then
  job job vk_${big_contexts}M_${name} ${big_mem} 32 NONE -- \
...
if [ ! -f $fst_file ] || [ $(stat stat -c %s ${fst_file}) -le 32 ] || [ $fst_file -ot $lm_file ]; then
  job rl_${big_contexts}M_${name} ${big_mem} 2 vk_${big_contexts}M_${name} -- \
    common/make_recog_lang.sh ${lm_file} ${langdir} ${rl_dir_big}
fi

#and carpa
carpa_file=${rl_dir_big}/G.carpa
if [ ! -f $carpa_file ] || [ $(stat stat -c %s ${carpa_file}) -le 32 ] || [ $carpa_file -ot $lm_file ]; then
  job ca_${big_contexts}M_${name} ${big_mem} 2 rl_${big_contexts}M_${name} -- \
...
graph_file=${graph_dir}/HCLGa.fst
if [ ! -f $graph_file ] || [ $(stat stat -c %s ${graph_file}) -le 32 ] || [ $graph_file -ot $rl_dir_small/G.fst ]; then
  job mk_${name} ${big_mem} 2 rl_${small_contexts}M_${name} -- \
    utils/mkgraph.sh --remove-oov --self-loop-scale 1.0 ${rl_dir_small} ${model} ${graph_dir}
fi
