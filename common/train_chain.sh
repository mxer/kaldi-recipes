...
tree_dir=exp/chain${dsuf}/tree${lsuf}
lat_dir=exp/chain${dsuf}/tri3${dsuf}${lsuf}_train${dsuf}_mc_sp_comb
dir=exp/chain${dsuf}/tdnn${lsuf}
train_data_dir=data/train${dsuf}_mc_sp_hires_comb
lores_train_data_dir=data/train${dsuf}_mc_sp_comb
train_ivector_dir=exp/ivectors_train${dsuf}_mc_sp_hires_comb


job align_lats 1 1 NONE -- steps/align_fmllr_lats.sh --nj 100 --cmd "$mfcc_cmd" ${lores_train_data_dir} data/lang${lsuf} $gmm_dir $lat_dir
