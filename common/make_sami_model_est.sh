...
model=morph_sami_v4_$(echo $lm | grep -o "N*W[0-9].*/N[0-9a-zA-Z\.]*" | sed "s#/#_#")
...
common/make_dict_est_morph.sh data/lm/$model/vocab $(dirname $(dirname $lm))/model.txt data/dicts/$model
...
sbatch common/recognize200.sh exp/chain_cleaned_sp/tdnn data/recog_langs/$model
sbatch common/recognize200.sh --dataset data/test exp/chain_cleaned_sp/tdnn data/recog_langs/$model
