...
if [ $# -lt 2 ]; then
   echo "usage: common/recognize.sh config recog_lang"
   echo "e.g.:  common/recognize.sh tdnn_a recog_lang"
...
graph=${dir}/graph_${sname}
mkgraph_flags="--self-loop-scale 1.0"
srun -n1 utils/mkgraph.sh --remove-oov $mkgraph_flags ${rl} ${dir} ${graph}

ivecs=exp/chain/dataprep/$dataprep/ivec/ivectors_$dsname
feats=exp/chain/dataprep/$dataprep/data/$dsname

srun -n1 steps/nnet3/decode.sh --iter $iter --beam 20.0 --lattice-beam 10.0 --skip-scoring $skip_scoring --nj 7 $decode_flags --scoring-opts "--min-lmwt 1" --online-ivector-dir $ivecs $graph $feats $dir/decode${iter}_${sname}${dsextra}
