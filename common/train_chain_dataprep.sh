...
if [ $speed_perturb ]; then
  job speed_perturb_train 1 4 LAST -- utils/data/perturb_data_dir_speed_3way.sh $data/orig/train $data/train
else
  job copy_h_train 1 4 LAST -- utils/copy_data_dir.sh $data/orig/train $data/train
fi

if [ $vol_perturb ]; then
  job vol_perturb_train 1 4 LAST -- utils/data/perturb_data_dir_volume.sh $data/train
fi

job copy_l_train 1 4 LAST -- utils/copy_data_dir.sh $data/train $data/train_lores
job val1_train 1 4 LAST   -- utils/validate_data_dir.sh --no-feats $data/train

numjobs=$(( $(wc -l < $data/orig/train/spk2utt) / 10 ))
job mfcc_lores_train 1 4 copy_l_train -- steps/make_mfcc.sh --cmd "$mfcc_cmd" --nj ${numjobs} $data/train_lores
job cmvn_lores_train 1 4 LAST         -- steps/compute_cmvn_stats.sh $data/train_lores
job fix_lores_train 4 4 LAST          -- utils/fix_data_dir.sh $data/train_lores
job val_lores_train 1 4 LAST          -- utils/validate_data_dir.sh $data/train_lores

for set in "train" "dev" "test"; do
  numjobs=$(( $(wc -l < $data/orig/$set/spk2utt) / 3 ))
  job mfcc_hires_$set 1 4 val1_$set -- steps/make_mfcc.sh --mfcc-config conf/${mfcc_conf}.conf --cmd "$mfcc_cmd" --nj ${numjobs} $data/$set
...
job ali 1 1  val_lores_train -- steps/align_fmllr.sh --nj $numjobs --cmd "slurm.pl --mem 2G" $data/train_lores data/lang $gmm $dir/ali
job lats 1 1 val_lores_train -- steps/align_fmllr_lats.sh --nj $(($numjobs*2)) --cmd "slurm.pl --mem 2G" $data/train_lores data/lang $gmm $dir/lats
...
job cmvn_comb 1 4 LAST      -- steps/compute_cmvn_stats.sh $data/train_comb
...
job max2_train 3 1 LAST              -- utils/data/modify_speaker_info.sh --utts-per-spk-max 2 $data/train_comb $ivec/data/train_comb_max2
...
numjobs=20
job ali_orig 1 1 LAST -- steps/align_fmllr.sh --nj $numjobs --cmd "slurm.pl --mem 2G" $data/orig/train data/lang $gmm $ivec/in_ali
...
job train_lda_mllt_iv 4 4 LAST \
 -- steps/train_lda_mllt.sh --cmd "$train_cmd" --num-iters 7 --mllt-iters "2 4 6" \
                            ...
                            3000 10000 $ivec/data/train data/lang \
