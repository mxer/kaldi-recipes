...
numjobs=10
SLURM_EXTRA_ARGS="-c ${numjobs}"
job ali_tri3${suf}_mc 1 4 tra_tri3${suf},utt2dur_train${suf}_mc \
 -- steps/align_si.sh  --nj ${numjobs} --cmd "$train_cmd"  data/train${suf}_mc data/lang exp/tri3${suf} exp/tri3${suf}_ali_mc

mkdir -p exp/ivector${suf}/tri5
job train_lda_mllt_iv${suf} 4 4 val_data_${set}_hires,ali_tri3${suf}_mc \
 -- steps/train_lda_mllt.sh --cmd "$train_cmd" --num-iters 7 --mllt-iters "2 4 6" \
                            --splice-opts "--left-context=3 --right-context=3" \
                            3000 10000 data/train${suf}_hires data/lang \
                            exp/tri3${suf}_ali_mc exp/ivector${suf}/tri5

mkdir -p exp/ivector${suf}/diag_ubm
job diag_ubm${suf} 4 4 train_lda_mllt_iv${suf},val_data_${set}_hires \
 -- steps/online/nnet2/train_diag_ubm.sh --cmd "$train_cmd" --nj $numjobs \
    --num-frames 700000 \
    --num-threads $numjobs \
    data/${set}_hires 512 \
    exp/ivector${suf}/tri5 exp/ivector${suf}/diag_ubm

job iv_extractor${suf} 8 24 LAST \
  -- steps/online/nnet2/train_ivector_extractor.sh --cmd "run.pl --max-jobs-run $[$numjobs/4]" --num-processes 1 --nj $numjobs \
    data/${set}_hires exp/ivector${suf}/diag_ubm exp/ivector${suf}/extractor
...
job iv_train${suf} 4 4 iv_extractor,max2${suf} \
 -- steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd" --nj $numjobs \
                                                  data/train${suf}_mc_sp_hires_comb_max2 exp/ivector${suf}/extractor \
                                                  exp/ivector${suf}/ivectors_train${suf}_mc_sp_hires_comb
#done
numjobs=5

SLURM_EXTRA_ARGS="-c ${numjobs}"
for set in "dev" "test"; do
  job iv_${set} 4 4 iv_extractor${suf},val_data_${set}_hires \
   -- steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd" --nj $numjobs \
                                                  data/${set}_hires exp/ivector${suf}/extractor \
                                                  exp/ivector${suf}/ivectors_${set}_hires
done
done
