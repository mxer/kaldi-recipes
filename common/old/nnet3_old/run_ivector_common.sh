...
if [ $stage -le 1 ]; then
  for datadir in "train" "eval" "dev"; do
    utils/copy_data_dir.sh data/$datadir data/${datadir}_hires
    steps/make_mfcc.sh --nj 40 --mfcc-config conf/mfcc_hires.conf \
      --cmd "$train_cmd" data/${datadir}_hires exp/make_hires/$datadir $mfccdir || exit 1;
    steps/compute_cmvn_stats.sh data/${datadir}_hires exp/make_hires/$datadir $mfccdir || exit 1;
  done
#  utils/subset_data_dir.sh --first data/train_hires 8000 data/train_8k_hires || exit 1
fi
...
if [ $stage -le 2 ]; then
  ...
  steps/align_fmllr.sh --nj 40 --cmd "$train_cmd" \
    data/train data/lang_train exp/tri4b exp/nnet3/tri4b_ali
fi
...
if [ $stage -le 3 ]; then
  ...
  steps/train_lda_mllt.sh --cmd "$train_cmd" --num-iters 13 \
    ...
    5000 10000 data/train_hires data/lang_train \
     exp/nnet3/tri4b_ali exp/nnet3/tri5b
fi

if [ $stage -le 4 ]; then

  steps/online/nnet2/train_diag_ubm.sh --cmd "$train_cmd" --nj 30 --num-threads 16 \
