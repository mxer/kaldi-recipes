...
if [ $stage -le 0 ]; then
  ...
  for wip in $(echo $word_ins_penalty | sed 's/,/ /g'); do
    ...
    $cmd LMWT=$min_lmwt:$max_lmwt $dir/scoring_kaldi/penalty_$wip/log/score_ler.LMWT.log \
      common/make_ler_file.py $dir/scoring_kaldi/penalty_$wip/LMWT.txt - \| \
      compute-wer --text --mode=present \
      "ark:common/make_ler_file.py $dir/scoring_kaldi/test_filt.txt - |"  ark,p:- ">&" $dir/ler_LMWT_$wip || exit 1;
  done
fi
