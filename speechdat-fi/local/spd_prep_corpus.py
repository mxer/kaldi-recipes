...
def main(in_dir, out_scp, out_dir):
    ...
    for key, path in FIO_files.items():
        ...
        try:
            ...
            print("Reading {} failed".format(path+".FIO"))
            continue

        chan = 1

        file_name = "{}.FIA".format(path)
        if not os.path.exists(file_name):
            print("Missing: {}".format(file_name))
            continue

        utt_key = "{}-ch{}-{}".format(speaker, chan, type)

        print("{} sox -X -e a-law -b 8 -r 8000 -t raw {} -b 16 -r 8000 -t wav - |".format(utt_key, file_name), file=fd_scp)
