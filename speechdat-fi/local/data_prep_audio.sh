...
if [ $# != 2 ]; then
   echo "usage: local/data_prep_audio.sh spd_dir data_prep_dir"
   echo "e.g.:  local/data_prep.sh \$GROUP_DIR/c/speechdat-fi/ data_prep_dir"
...
local/spd_prep_corpus.py ${spd_dir} ${raw_files_dir}/wav.scp ${data_dir}
