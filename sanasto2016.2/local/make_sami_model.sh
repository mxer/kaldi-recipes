...
model=morph_sami_$(echo $lm | grep -o "N*W[0-9].*/N[0-9a-zA-Z\.-]*" | sed "s#/#_#")
...
common/make_dict.sh data/lm/$model/vocab data/dicts/$model
...
utils/mkgraph.sh --remove-oov --self-loop-scale 1.0 data/recog_langs/$model exp/chain_cleaned_mc1/tdnn exp/chain_cleaned_mc1/tdnn/graph_$model

sbatch common/recognize100.sh exp/chain_cleaned_mc1/tdnn data/recog_langs/$model
sbatch common/recognize100.sh --dataset data/test exp/chain_cleaned_mc1/tdnn data/recog_langs/$model
